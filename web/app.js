...
import {
  ageInDays, catalogUrl, day, durabilityTag, esc, humanSize,
  makeRunner, table,
} from './view.js';
...
let run;                  // set once the connection is open
...
async function boot() {
  ...
  try {
    ...
    const url = await catalogUrl(config, location, resolveFeed);
    db = await open(url, { verify: config.verify ?? false });
    run = makeRunner((name, params = []) => db.query(Q[name], params));
